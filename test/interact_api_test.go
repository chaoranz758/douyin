...
func TestFavorite(t *testing.T) {
	...
	firstVideo := feedResp.Value("video_list").Array().Last().Object()
	videoId1 := firstVideo.Value("id").Number().Raw()
	videoId := int64(videoId1)

	userId, token := getTestUserToken(testUserA+"33345", e)
...
func TestComment(t *testing.T) {
	...
	_, token := getTestUserToken(testUserA+"23309", e)
