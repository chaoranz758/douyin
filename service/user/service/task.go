...
const (
	...
	successPushActiveSetAndString   = "push active set and string success"
...
func PushActiveSetAndString() error {
	...
	res, err := client.FollowClient.GetFollowFollowerIdList(context.Background(), &request.DouyinGetFollowFollowerIdListRequest{
		UserId: userId,
	})
	if err != nil {
		if res == nil {
	...
	//存视频相关信息
	res1, err := client.VideoClient.PushActiveBasicInfoInit(context.Background(), &request1.DouyinPushActiveInfoInitRequest{
		UserId: userId,
	})
	if err != nil {
		if res1 == nil {
